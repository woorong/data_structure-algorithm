...
public class Graph
{
	...
	public void contractNodeWithTwoEdges(int node)
		throws GraphException
	{
		if(!graph.containsKey(node) || graph.get(node).size() != 2) {
			throw new GraphException();
		}

		// WRITE YOUR CODE HERE
		List<Connection> edgeList = graph.get(node);
		int a = edgeList.get(0).getNode();
		int b = edgeList.get(1).getNode();
		addEdge(a, b, edgeList.get(0).getDistance() + edgeList.get(1).getDistance());

		edgeList = graph.get(a);
		for (Connection connection : edgeList) {
			if(connection.getNode() == node) {
				edgeList.remove(connection);
				break;
			}
		}
		edgeList = graph.get(b);
		for (Connection connection : edgeList) {
			if(connection.getNode() == node) {
				edgeList.remove(connection);
				break;
		...
		throws GraphException
	{
		...
		//initialize the shortest distance from node1 to other nodes
		Map<Integer, Integer> distances = new HashMap<>();
		for(Integer node : graph.keySet()) {
			if(node != node1) {
				if(connected(node, node1)) {
					distances.put(node, distance(node, node1));
				}else {
		...
		while(distances.keySet().size() > 0) {
			//find the min node from remain nodes
			int minDistance = Integer.MAX_VALUE;
			int minNode = -1;
			for(int n : distances.keySet()) {
			...
			//update the distances from minNode
			List<Connection> edgeList = graph.get(minNode);
			for (Connection connection : edgeList) {
				if(distances.containsKey(connection.getNode()) &&
						connection.getDistance() + distances.get(minNode)
						< distances.get(connection.getNode())) {
					distances.put(connection.getNode(), connection.getDistance() + distances.get(minNode) );
				}
			}
	...
	/**
	 * check if the two nodes connects to each other
	 * @param node1
	 * @param node2
